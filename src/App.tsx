...
export function App() {
  ...
  // Only these two take part in the push/pop. Prompt Mode and the lab sit outside
  // AnimatePresence entirely, and that placement is the whole safety argument: flipping to
  // 'prompt' unmounts the animated subtree wholesale, so no exit animation can run and no
  // screen can outlive the presenter's Exit — the microphone and the wake lock stay bound to
  // PromptScreen's unmount, exactly as before.
  const inFlow = view === 'editor' || view === 'setup'

  // With only two screens in the flow, the destination *is* the direction.
  const dir = view === 'setup' ? 1 : -1

  // MotionConfig wraps *everything*, Prompt Mode included. It has no motion components today,
  // so this changes nothing now — but the alternative is a root guard that silently does not
  // cover the one screen where the reduced-motion invariant actually matters.
  return (
    <MotionConfig reducedMotion="user">
      {view === 'prompt' && <PromptScreen />}
      {view === 'lab' && <SmartFollowLabScreen />}
      {inFlow && (
